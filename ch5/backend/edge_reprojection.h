 ...
 */
class EdgeReprojection : public Edge {// 构建这条误差边，需要初始化第i和第j个路标点的相机系观测（归一化点）
public:
...
*/
class EdgeReprojectionXYZ : public Edge {   // 构建这条误差边，需要初始化路标点的相机系观测（归一化点）
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
