...
void EdgeReprojection::ComputeResidual() {
    ...
    // 这里实际就是VINS中基于逆深度的重投影误差!!!! vio第3讲第31页
    Vec3 pts_camera_i = pts_i_ / inv_dep_i;     // 路标点
    Vec3 pts_imu_i = qic * pts_camera_i + tic;  // 外参Tbc，这里其实简化和cam1是一个坐标系
    Vec3 pts_w = Qi * pts_imu_i + Pi;   // Pw = Rwc1*Pc1 + twc1
    Vec3 pts_imu_j = Qj.inverse() * (pts_w - Pj);   // Pc2 = Rcw2 * (Pw - twc2)
    Vec3 pts_camera_j = qic.inverse() * (pts_imu_j - tic);  // tic = 0 qic = I
...
void EdgeReprojection::ComputeJacobians() {
    ...
    // 这个就是经典的视觉SLAM残差求导过程，只不过这里是vio里面结合逆深度投影的雅可比
    // 首先对投影点求导2*3 [1/Z 0 -X/Z^2; 0 1/Z -Y/Z^2]
    Mat23 reduce(2, 3);
...
//    reduce = information_ * reduce;
    ...
    /*******i时刻*********/
    Eigen::Matrix<double, 2, 6> jacobian_pose_i;    // 对于位姿v的最终雅可比2*6
    Eigen::Matrix<double, 3, 6> jaco_i; // 投影点对R t求导3*6
    // 公式64，对i时刻平移量
    ...
    /*******j时刻*********/
    Eigen::Matrix<double, 2, 6> jacobian_pose_j;    // 对于位姿j的最终雅可比2*6
...
void EdgeReprojectionXYZ::ComputeResidual() {
    ...
    Vec3 pts_imu_i = Qi.inverse() * (pts_w - Pi);   // Pc = Rcw*(Pw - twc)
    Vec3 pts_camera_i = qic.inverse() * (pts_imu_i - tic);  // qic = E  tic = 0

    double dep_i = pts_camera_i.z();
    // 因为有的是真实数据，所以观测（归一化）和路标点（三维点）顶点都需要加噪声
...
void EdgeReprojectionXYZ::ComputeJacobians() {
    ...
    Mat33 Ri = Qi.toRotationMatrix();   // Rwc
    Mat33 ric = qic.toRotationMatrix();
    Mat23 reduce(2, 3);
    ...
    jaco_i.leftCols<3>() = ric.transpose() * -Ri.transpose();   // -Rcw
    jaco_i.rightCols<3>() = ric.transpose() * Sophus::SO3d::hat(pts_imu_i); // pts_imu_i反对称矩阵
    jacobian_pose_i.leftCols<6>() = reduce * jaco_i;
